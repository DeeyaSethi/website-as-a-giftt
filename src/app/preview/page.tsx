...
import { ArrowLeft, Download, Share2, RefreshCw, Loader2, Pencil } from "lucide-react";
...
import { EditContentModal } from "@/client/components/preview/EditContentModal";
...
export default function PreviewPage() {
  ...
  const handleUpdateContent = (updatedData: GeneratedSiteConfig) => {
    // 1. Update state
    setSiteData(updatedData);

    // 2. Update session storage so iframe finds new data
    // Maintain outer structure if it exists
    const currentStorage = sessionStorage.getItem("generatedSite");
    let newStorageData;
    if (currentStorage) {
      const parsed = JSON.parse(currentStorage);
      if (parsed.site) {
        newStorageData = { ...parsed, site: updatedData };
      } else {
        newStorageData = updatedData;
      }
    } else {
      newStorageData = updatedData;
    }
    sessionStorage.setItem("generatedSite", JSON.stringify(newStorageData));

    // 3. Force iframe reload
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              ...
              <span className="hidden sm:inline">Back / New</span>
            </button>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm text-slate-600 hover:text-slate-900 transition-colors bg-slate-100 hover:bg-slate-200 rounded-lg"
              >
